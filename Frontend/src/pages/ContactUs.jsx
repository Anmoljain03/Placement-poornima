...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${import.meta.env.VITE_LIVE_URL}/api/auth/contact`, formData, {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-6 py-12 max-w-8xl mx-auto">
        {[
          ...
          <motion.div
            ...
            className={`bg-white p-6 shadow-lg rounded-lg border flex flex-col gap-4 relative cursor-pointer transition-all duration-300 hover:shadow-2xl ${selected === index
          ...
          >
            {/* Right-side dot */}
            <div
              className={`absolute top-4 right-4 w-3 h-3 rounded-full transition-all duration-300 ${selected === index ? "bg-[#1a365d]" : "bg-gray-300"
                }`}
            ></div>
      ...
      <div className="w-full max-w-5xl mx-auto p-6 ">
        ...
        <div className="flex border-b border-gray-300 ">
          <button
            ...
            className={`flex-1 py-3 text-center text-lg font-semibold  ${activeTab === "faq"
          ...
          <button
            ...
            className={`flex-1 py-3 text-center text-lg font-semibold ${activeTab === "howUsed"
                ? "border-b-2 border-[#1a365d] bg-gradient-to-r from-[#102a43] to-[#486581] bg-clip-text text-transparent"
                : "text-gray-500"
              }`}
          >
