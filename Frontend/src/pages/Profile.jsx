...
const Profile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_LIVE_URL}/api/auth/update-profile`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: token,
    ...
    <div className="p-6 mx-auto bg-white shadow-lg">
      ...
      <div className="flex flex-wrap justify-center gap-4 bg-white rounded-lg p-3 shadow-md w-[95%] sm:w-[80%] mx-auto">
        ...
        {[
          ...
          <button
            key={tab}
            className={`flex items-center space-x-2 px-8 py-3 text-lg font-medium rounded-lg transition-all duration-300 ${activeTab === tab
